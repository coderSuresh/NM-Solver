...
export function jacobi(params: JacobiParams): JacobiResult {
    ...
    const iterationSteps: JacobiIterationStep[] = [];
    let iteration = 0;
    ...
    do {
        ...
        // Compute next approximation (Jacobi uses old values for all calculations)
        ...
        // Add current iteration to steps array (before updating)
        iterationSteps.push({
            iteration,
            x: xNew,
            y: yNew,
        ...
        table.push([
            iteration,
            customFormatNumber(xNew),
            customFormatNumber(yNew),
            customFormatNumber(zNew),
